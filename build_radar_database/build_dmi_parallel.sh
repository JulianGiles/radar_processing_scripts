...
for file in ${path}/*${location}*.tar.gz; do
    if [ -f "$file" ]; then
        ...
        srun -c 8 --account=detectrea -n 1 --exact --threads-per-core=1 ./build_dmi_parallel_step.sh ${include_years} ${include_months} ${concat_to_daily} ${path} ${location} ${transfer} ${jsc_folder} ${overwrite} ${file} &
