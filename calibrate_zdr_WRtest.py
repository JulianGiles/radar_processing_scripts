...
if __name__ == "__main__": # set guard
    ...
    # to check WR at different Zm levels
    Zm_WR1015 = (10, 15)
    Zm_WR1520 = (15, 20)
    Zm_WR2025 = (20, 25)
    Zm_WR2530 = (25, 30)
    ...
    def make_savedir(ff, name):
        ...
        savepath = (country+"/calibration_WRtest/zdr/"+name+"/").join(ff_parts)
    ...
    for ff in files:
        ...
        if not overwrite:
            ...
            if 2 in calib_types:
                ...
                if len(os.listdir(os.path.dirname(savepath))) >= 20: # this number must match the min expected outputs (see how many below)
                    calib_2 = False
            else: calib_2 = False
            ...
            if 3 in calib_types:
                ...
                if ( len(os.listdir(os.path.dirname(savepathqvp))) >= 18 ): # this number must match the min expected outputs (see how many below)
        ...
        if 1 in calib_types or 2 in calib_types or 3 in calib_types:
            ...
            if 2 in calib_types and calib_2:
                ...
                if "height_ml_bottom_new_gia" in data:
                    ...
                    # Calculate offset below ML for full timespan wet radome levels
                    for Zm_WRX in [Zm_WR1015, Zm_WR1520, Zm_WR2025, Zm_WR2530, Zm_WR3035, Zm_WR35]:
                        try:
                            zdr_offset = utils.zhzdr_lr_consistency(data.sel(time=data.time.where(data["Zm"]>Zm_WRX[0], drop=True).where(data["Zm"]<Zm_WRX[1], drop=True)),
                                                                    zdr=X_ZDR, dbzh=X_DBZH, rhohv=X_RHO,
                                                                    mlbottom="height_ml_bottom_new_gia_clean",
                                                                    min_h=min_height, timemode="all", band=band)
                        ...
                        # save the arrays
                        savepath = make_savedir(ff, "LR_consistency")
                        filename = ("zdr_offset_belowML_WR"+str(Zm_WRX[0])+str(Zm_WRX[1])).join(savepath.split(separator))
                        zdr_offset.to_netcdf(filename)
                ...
                for Zm_WRX in [Zm_WR1015, Zm_WR1520, Zm_WR2025, Zm_WR2530, Zm_WR3035, Zm_WR35]:
                    ...
                    # save the arrays
                    savepath = make_savedir(ff, "LR_consistency")
                    filename = ("zdr_offset_below1C_WR"+str(Zm_WRX[0])+str(Zm_WRX[1])).join(savepath.split(separator))
                    zdr_offset.to_netcdf(filename)
                ...
                for timemode in ["all"]:
                    ...
                    for azmed in [True]: # False can be added if we want a non-QVP calculation
                        if "height_ml_bottom_new_gia" in data:
                            ...
                            for Zm_WRX in [Zm_WR1015, Zm_WR1520, Zm_WR2025, Zm_WR2530, Zm_WR3035, Zm_WR35]:
                                try:
                                    zdr_offset = utils.zdr_offset_detection_qvps(data.sel(time=data.time.where(data["Zm"]>Zm_WRX[0], drop=True).where(data["Zm"]<Zm_WRX[1], drop=True)),
                                                                            zdr=X_ZDR, dbzh=X_DBZH, rhohv=X_RHO, azmed=azmed,
                                                                            mlbottom="height_ml_bottom_new_gia_clean",
                                                                            min_h=min_height, timemode=timemode, minbins=minbins).compute()
                        ...
                        # Calculate offset below 1C for full timespan wet radome levels
                        for Zm_WRX in [Zm_WR1015, Zm_WR1520, Zm_WR2025, Zm_WR2530, Zm_WR3035, Zm_WR35]:
                            try:
                                zdr_offset = utils.zdr_offset_detection_qvps(data.sel(time=data.time.where(data["Zm"]>Zm_WRX[0], drop=True).where(data["Zm"]<Zm_WRX[1], drop=True)),
                                                                        zdr=X_ZDR, dbzh=X_DBZH, rhohv=X_RHO, mlbottom=1, azmed=azmed,
